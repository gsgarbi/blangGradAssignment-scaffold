...
public class BipartiteMatchingSampler implements Sampler {
  ...
  public void execute(Random rand) {
	  ...
	  qAfterGivenBefore, qBeforeGivenAfter, logQAfterGivenBefore, logQBeforeGivenAfter;
	  int numConnected;
	  ...
	  // and get q's if a connection was not made
		  ...
		  qAfterGivenBefore =
				  Math.sqrt((1/matching.componentSize()));
		  numConnected = matching.componentSize() - matching.free1().size();
		  normF =
				  Math.exp(SpecialFunctions.logBinomial(numConnected, 2)) + 
				  matching.free1().size();
	  ...
	  if (connectOp) {
		  ...
		  qAfterGivenBefore =
				  Math.sqrt(1/(matching.componentSize() * matching.free2().size()));
		  numConnected = matching.componentSize() - matching.free1().size();
		  normF =
				  Math.exp(SpecialFunctions.logBinomial(numConnected, 2)) + 
				  matching.free1().size();
